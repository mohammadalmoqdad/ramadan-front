...
export default function EditCompetitionForm({contest, setContest}) {
    ...
    const [classNewAnnouncementMessagesColor, setNewAnnouncementMessagesClassColor] = useState("");
    ...
    const resetForm = () => {
        setAnnouncements(contest.announcements && contest.announcements.length > 0 ? [...contest.announcements] : []);
    ...
    const deleteAnnouncementHandler = (index) => {
        let tmpAnnouncements = announcements.filter((announcement, i) => i !== index);
        ...
        updateContest(
            ...
            (res) => {
                if (res?.status === 200) {
                    ...
                    setNewAnnouncementMessages([t("contest-has-been-edited-successfully")]);
    ...
    const changeAnnouncementTextHandler = (e, index) => {
        let tmpAnnouncements = announcements.map(
            (announcement, i) => i === index ? e.target.value : announcement
        );
        setAnnouncements(tmpAnnouncements);
    };
                ...
                <ParticipantsMember2>
                    ...
                    <ParticipantsNumbers>
                        ...
                        {newAnnouncement.length > 0 &&
                            <ButtonStyle onClick={handleAddNewAnnouncementClick}> {t("add")}</ButtonStyle>
                        }
                        {newAnnouncementMessages.length > 0 &&
                            newAnnouncementMessages.map((message, index) => {
                                return <DivPass className={classNewAnnouncementMessagesColor}
                ...
                <ParticipantsMember2>
                    <ParticipantsNumbers>
                        ...
                        <OverflowScrolling>
                            ...
                            <InputField type={"text"} label={t("name-label")} onChange={handleNameChange} value={name}/>
                            <InputField type={"text"} label={t("description-label")} onChange={handleDescriptionChange} value={description}/>
                            <InputField type={"checkbox"} label={t("active-announcements")} checked={showStanding} onChange={handleShowStandingChange}/>
                            <InputField type={"checkbox"} label={t("readonly")} checked={readOnlyMode} onChange={handleReadOnlyChange}/>
                                    ...
                                    {announcements.map((announcement, index) => (
                                        <TypeSpace key={index} inside={true}>
                                            ...
                                            <TextAreaSpace
                                                ...
                                                onChange={(e, index) => changeAnnouncementTextHandler(e, index)}
                                            >{announcement}</TextAreaSpace>
                                            <PublishedDate>{t("announcement") + " " + (index + 1)}</PublishedDate>
                                        </TypeSpace>
                                    ))}
                                </div>
                            ) : (
                                <ParticipantsTitelsAtHome>{t("no-announcements-yet")}</ParticipantsTitelsAtHome>
                            )}
                            {messages.length > 0 &&
